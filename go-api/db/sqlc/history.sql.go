...
const getInvoicesByDateReverse = `-- name: GetInvoicesByDateReverse :many
SELECT 
    DATE_TRUNC('day', created_at)::timestamp AS issued_date,
    COUNT(*) AS num_invoices, 
    JSON_AGG(invoice_data) AS invoice_data
...
type GetInvoicesByDateReverseRow struct {
	IssuedDate  pgtype.Timestamp `json:"issued_date"`
	NumInvoices int64            `json:"num_invoices"`
	InvoiceData []byte           `json:"invoice_data"`
}

func (q *Queries) GetInvoicesByDateReverse(ctx context.Context) ([]GetInvoicesByDateReverseRow, error) {
	rows, err := q.db.Query(ctx, getInvoicesByDateReverse)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetInvoicesByDateReverseRow{}
	for rows.Next() {
		var i GetInvoicesByDateReverseRow
		if err := rows.Scan(&i.IssuedDate, &i.NumInvoices, &i.InvoiceData); err != nil {
			return nil, err
		}
		items = append(items, i)
